...
ADVANCED ML ANALYSIS â€” MENTAL HEALTH (disu) as Dependent Variable
...
Target     : disu (Mental Health Disutility/Disability) â†’ Binary (0/1)
...
DEPENDENT VARIABLES (Mental Health Outcomes):
  dep            = Depression
  anx            = Anxiety
  disu           = Mental Health Disutility / Disability (THIS ANALYSIS)
...
print("=" * 70)
print("  BDHS 2022 â€” ADVANCED MENTAL HEALTH (disu) ANALYSIS (Q1 Standard)")
print("=" * 70)
...
OUTPUT_DIR    = r"e:\hafiza mam work\advanced_analysis\results_disu"
os.makedirs(OUTPUT_DIR, exist_ok=True)

TARGET        = 'disu'
TARGET_LABEL  = 'Mental Health (disu)'
OTHER_TARGETS = ['dep', 'anx']
    ...
    for var in [v for v in VAR_LABELS if v in df_imp.columns]:
        ...
        for cat_val in sorted(df_imp[var].dropna().unique()):
            ...
            weighted_desc.append({
                ...
                f'{TARGET}_Prev_Weighted_%': round(
...
print(f"  â†’ {len(selected_features)} features selected")
...
def bootstrap_metrics(y_true, y_pred, y_prob, n_bootstrap=1000, rng=None):
    ...
    for metric, key in [('Accuracy','acc'),('Precision','prec'),
        ...
        m,l,h = ci(boot[key])
        r[f'{metric}_mean'] = m; r[f'{metric}_CI_lower'] = l; r[f'{metric}_CI_upper'] = h
    return r
...
for ax, (mn, mc, lc, hc, col) in zip(axes, metrics_to_vis):
    model_names = bs_df['Model'].values
    means = bs_df[mc].values; lows = bs_df[lc].values; highs = bs_df[hc].values
    bars = ax.bar(model_names, means, color=col, alpha=0.80, edgecolor='black', linewidth=0.8)
    ax.errorbar(range(len(model_names)), means,
    ...
    for bar, m, lo, hi in zip(bars, means, lows, highs):
        ax.text(bar.get_x()+bar.get_width()/2, hi+0.5,
                f'{m:.1f}\n[{lo:.1f}â€“{hi:.1f}]',
...
def bootstrap_auc_diff(y_true, y_prob1, y_prob2, n=1000, seed=42):
    ...
    for _ in range(n):
        ...
        if len(np.unique(yt)) < 2: continue
        try:
            diffs.append(roc_auc_score(yt,np.array(y_prob1)[idx]) -
                         roc_auc_score(yt,np.array(y_prob2)[idx]))
        except: pass
    diffs = np.array(diffs)
    obs   = roc_auc_score(y_true,y_prob1) - roc_auc_score(y_true,y_prob2)
    p     = 2*min(np.mean(diffs>=obs), np.mean(diffs<=obs))
    return obs, diffs, p
...
pd.DataFrame({
    ...
    'Model_1': [best_name]*2, 'Model_2': [second_name]*2,
...
plt.hist(diff_dist, bins=50, color='#5C6BC0', edgecolor='black',
         alpha=0.8, density=True, label='Bootstrap Î” AUC')
...
plt.xlabel('Î” AUC', fontsize=11); plt.ylabel('Density', fontsize=11)
...
ax.set_title(f'ROC Curves â€” {TARGET_LABEL} (BDHS 2022)\nAUC with 95% Bootstrap CI',
...
# SHAP ANALYSIS
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
print(f"\n[STEP 7] SHAP Analysis â€” {best_name}...")
feat_labels = [VAR_LABELS.get(c,c) for c in selected_features]
best_uses_scale = (best_name in NEEDS_SCALING)
X_test_for_shap = (pd.DataFrame(X_test_scaled, columns=selected_features)
...
if SHAP_AVAILABLE:
    try:
        if best_name in ["XGBoost","Random Forest","Decision Tree"]:
            # Use sampling for SHAP if test set is large to prevent hangs
            X_shap = X_test_for_shap
            if len(X_shap) > 500:
                X_shap = X_shap.sample(500, random_state=RANDOM_SEED)
            ...
            if isinstance(sv_raw, list):
                ...
                if sv_raw.ndim == 3: # (samples, features, classes) -> Select class 1
                    sv = sv_raw[:, :, 1]
                else:
                    sv = sv_raw
            else:
                sv = sv_raw
            X_test_for_shap = X_shap # Update to sampled version for plotting
        else:
            # Use sampling for SHAP if test set is large
            X_shap = X_test_for_shap
            if len(X_shap) > 500:
                X_shap = X_shap.sample(500, random_state=RANDOM_SEED)
            ...
            sv_raw_lin  = explainer.shap_values(X_shap)
            sv          = sv_raw_lin[1] if isinstance(sv_raw_lin,list) else sv_raw_lin
            X_test_for_shap = X_shap # Update to sampled version for plotting

        shap_abs = np.abs(sv).mean(axis=0)
        shap_bar = pd.DataFrame({'Feature':feat_labels,'Mean_|SHAP|':shap_abs})\
                     .sort_values('Mean_|SHAP|',ascending=True)

        plt.figure(figsize=(10, max(7, len(feat_labels)*0.45)))
        colors_s = plt.cm.RdYlBu_r(np.linspace(0.2, 0.9, len(shap_bar)))
        bars = plt.barh(shap_bar['Feature'], shap_bar['Mean_|SHAP|'],
                        color=colors_s, edgecolor='black', linewidth=0.7)
        plt.xlabel('Mean |SHAP Value|', fontsize=11)
        plt.title(f'SHAP Feature Importance â€” {best_name}\n({TARGET_LABEL}, BDHS 2022)',
                  fontsize=12, fontweight='bold')
        for bar, val in zip(bars, shap_bar['Mean_|SHAP|']):
        ...
        shap_exp = shap.Explanation(values=sv, base_values=np.full(len(sv),base_val),
        ...
        top3_feats = shap_bar.tail(3)['Feature'].tolist()[::-1]
        ...
        plt.suptitle(f'SHAP Dependence â€” Top {TARGET_LABEL} Predictors',
        ...
        shap_bar.sort_values('Mean_|SHAP|',ascending=False).to_csv(
            f"{OUTPUT_DIR}/shap_feature_importance.csv", index=False)
        print(f"  [Saved] SHAP outputs")
...
print(f"\n  BEST MODEL ({TARGET_LABEL}): {best_row['Model']}")
